...
function setupEventHandlers(orchestrationService, jiraService, progressService, webhookService, notificationService, logger) {
    // Listen for workflow events from orchestrationService
        ...
        switch (event.type) {
            case 'started':
                ...
                notificationService.showNotification({
                    ...
                    message: `Workflow ${event.workflowId} started.`
                });
                break;
            case 'step-completed':
                ...
                break;
            case 'step-failed':
                logger.error(`Workflow ${event.workflowId} step ${event.stepId} failed: ${event.data?.error}`, 'Workflow');
                notificationService.showNotification({
                    type: 'error',
            ...
            case 'completed':
                ...
                notificationService.showNotification({
                    ...
                    message: `Workflow ${event.workflowId} completed.`
                });
                break;
            case 'failed':
                logger.error(`Workflow ${event.workflowId} failed: ${event.data?.error}`, 'Workflow');
                notificationService.showNotification({
                    type: 'error',
                ...
                break;
            default:
                logger.debug(`Unhandled workflow event: ${event.type}`, 'Workflow');
        }
    });
    ...
    webhookService.on('jiraTaskUpdated', async (payload) => {
        logger.info('Jira task updated webhook received', 'Webhook');
        // Handle task updates if needed
    });
    webhookService.on('gitPrMerged', async (payload) => {
        logger.info('Git PR merged webhook received', 'Webhook');
        // Handle PR merge events if needed
    });
}
function registerCommands(context, orchestrationService, jiraService, llmService, gitService, testingService, progressService, configManager, backupService) {
    const logger = logger_1.Logger.getInstance();
    const errorHandler = errorHandler_1.ErrorHandler.getInstance();
    const commands = [
        ...
        vscode.commands.registerCommand('aiDevAssistant.startWorkflow', async () => {
            try {
                ...
                if (taskKey) {
                    ...
                    vscode.window.showErrorMessage('No Jira task key provided');
                }
            }
            catch (error) {
            ...
            try {
                ...
                if (workflowId) {
                    ...
                    if (failedStep) {
                        ...
                        vscode.window.showWarningMessage('No failed steps found in workflow');
                    }
                }
            }
            catch (error) {
            ...
            try {
                ...
                if (taskKey) {
                    ...
                    if (task) {
                        ...
                        const codeResult = await llmService.generateCode(codeRequest);
                        if (codeResult && codeResult.files && codeResult.files.length > 0) {
                            await vscode.workspace.openTextDocument({ content: codeResult.files[0].content, language: codeResult.files[0].language || 'typescript' });
        ...
        vscode.commands.registerCommand('aiDevAssistant.reviewCode', async () => {
            try {
                ...
                vscode.window.showErrorMessage(`Failed to review code: ${errorMessage}`);
            }
        }),
        vscode.commands.registerCommand('aiDevAssistant.validateConfig', async () => {
            try {
                const isValid = await (0, validationService_1.validateCredentials)();
                if (isValid) {
        ...
        vscode.commands.registerCommand('aiDevAssistant.backupData', async () => {
            try {
                await backupService.createBackup('full', 'Manual backup');
        ...
        vscode.commands.registerCommand('aiDevAssistant.restoreData', async () => {
            try {
                ...
                const selected = await vscode.window.showQuickPick(backupFiles.map(f => ({ label: f.id, description: f.timestamp ? new Date(f.timestamp).toLocaleString() : f.description })), { placeHolder: 'Select backup to restore' });
                if (selected) {
                    const confirm = await vscode.window.showWarningMessage('This will restore data from the selected backup. Continue?', 'Yes', 'No');
                    if (confirm === 'Yes') {
                        await backupService.restoreBackup(selected.label);
                        vscode.window.showInformationMessage('Data restored successfully');
                    }
                }
            }
            catch (error) {
...
async function viewProgressDashboard(progressService) {
    ...
    const workflows = await progressService.getActiveWorkflows();
    ...
    <body>
      ...
      ${workflows.map(workflow => `
        <div class="workflow">
          ...
          <p>Task: ${workflow.jiraTask?.key || workflow.taskKey}</p>
...
async function selectWorkflowForRetry(progressService) {
    ...
    const selected = await vscode.window.showQuickPick(failedWorkflows.map((w) => ({ label: w.id, description: w.jiraTask?.key || w.taskKey })), { placeHolder: 'Select workflow to retry' });
    return selected?.label;
}
async function selectWorkflowForCancellation(progressService) {
    const workflows = await progressService.getActiveWorkflows();
    const activeWorkflows = workflows.filter((w) => w.status === 'in-progress' || w.status === 'pending' || w.status === 'running');
    if (activeWorkflows.length === 0) {
        vscode.window.showInformationMessage('No active workflows to cancel');
        return undefined;
    }
    const selected = await vscode.window.showQuickPick(activeWorkflows.map((w) => ({ label: w.id, description: w.jiraTask?.key || w.taskKey })), { placeHolder: 'Select workflow to cancel' });
    return selected?.label;
}
...
//# sourceMappingURL=extension-final.js.map