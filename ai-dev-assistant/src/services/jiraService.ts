...
import { IJiraService, JiraTask, JiraConfig } from '../types';
...
export class JiraService implements IJiraService {
	...
	private loadConfiguration(): JiraConfig {
	...
	private async startOAuth2Flow(): Promise<boolean> {
		try {
	...
	private async exchangeCodeForTokens(authCode: string): Promise<any> {
	...
	private async loadStoredTokens(): Promise<{ accessToken: string; refreshToken: string } | null> {
		const extensionContext = (global as any).extensionContext;
	...
	public async getTasks(projectKey?: string, assignee?: string): Promise<JiraTask[]> {
		...
		try {
			...
			return response.data.issues.map((issue: any) => this.mapJiraIssueToTask(issue));
		} catch (error: unknown) {
			if (error instanceof Error) {
	...
	public async getTask(taskKey: string): Promise<JiraTask> {
		if (!this.isAuthenticated()) {
			throw new Error('Not authenticated with Jira');
		}
	...
	public async updateTaskStatus(taskKey: string, status: string, comment?: string): Promise<boolean> {
		if (!this.isAuthenticated()) {
			throw new Error('Not authenticated with Jira');
		}
	...
	public async addComment(taskKey: string, comment: string): Promise<boolean> {
		if (!this.isAuthenticated()) {
			throw new Error('Not authenticated with Jira');
		}
	...
	public async createSubtask(parentKey: string, summary: string, description: string): Promise<JiraTask> {
		if (!this.isAuthenticated()) {
			throw new Error('Not authenticated with Jira');
		}
	...
	private mapJiraIssueToTask(issue: any): JiraTask {
		return {
			id: issue.id,
			key: issue.key,
