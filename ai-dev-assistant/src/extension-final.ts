...
import { WorkflowEvent, JiraTask } from './types';
...
function setupEventHandlers(
	...
	webhookService.on('jiraTaskUpdated', async (payload: any) => {
		logger.info('Jira task updated webhook received', 'Webhook');
		// Handle task updates if needed
	});

	webhookService.on('gitPrMerged', async (payload: any) => {
		...
		vscode.commands.registerCommand('aiDevAssistant.restoreData', async () => {
			try {
				...
				const selected = await vscode.window.showQuickPick(
					backupFiles.map(f => ({ label: f.id, description: f.timestamp ? new Date(f.timestamp).toLocaleString() : f.description })),
					{ placeHolder: 'Select backup to restore' }
				);
				if (selected) {
					const confirm = await vscode.window.showWarningMessage(
						'This will restore data from the selected backup. Continue?',
						'Yes', 'No'
					);
					if (confirm === 'Yes') {
...
async function selectWorkflowForRetry(progressService: ProgressTrackingService): Promise<string | undefined> {
	...
	const selected = await vscode.window.showQuickPick(
		failedWorkflows.map((w: any) => ({ label: w.id, description: w.jiraTask?.key || w.taskKey })),
...
async function selectWorkflowForCancellation(progressService: ProgressTrackingService): Promise<string | undefined> {
	const workflows = await progressService.getActiveWorkflows();
	const activeWorkflows = workflows.filter((w: any) => w.status === 'in-progress' || w.status === 'pending' || w.status === 'running');
	if (activeWorkflows.length === 0) {
		vscode.window.showInformationMessage('No active workflows to cancel');
		return undefined;
	}
	const selected = await vscode.window.showQuickPick(
		activeWorkflows.map((w: any) => ({ label: w.id, description: w.jiraTask?.key || w.taskKey })),
